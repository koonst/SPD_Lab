...
class RPQ_Instance:
    ...
    def load_from_file(file_path):
        ...
        f = open(file_path, "r")
        czyt_linia = f.readline()
        czyt_linia_split = czyt_linia.split()
        liczba_zadan = int(czyt_linia_split[0])
        instancja = RPQ_Instance()
        #instancja.tasks_number = 3
        instancja.tasks_number = liczba_zadan
        for i in range(liczba_zadan):
            czyt_linia = f.readline()
            czyt_linia_split = czyt_linia.split()

            term_dost = int(czyt_linia_split[0])
            czas_wyk = int(czyt_linia_split[1])
            czas_dost = int(czyt_linia_split[2])
            rpq = RPQ_Task(i, term_dost, czas_wyk, czas_dost)
            instancja.tasks.append(rpq)
        return instancja
...
if __name__ == '__main__':
    ...
    for i in data:
        plik = "plik_dane" + str(i) + ".txt"

        start = time.perf_counter()

    #test_instance = RPQ_Instance.load_from_file("plik_dane0.txt")
    #cmax, pi_order, status = solve_rpq_with_solver(test_instance)


        test_instance = RPQ_Instance.load_from_file(plik)
        cmax, pi_order, status = solve_rpq_with_solver(test_instance)
